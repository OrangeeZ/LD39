...
{
    private string SaveFolderPath = "Assets/Resources/Environment/";
    ...
    void ResetMap()
    {
        basicMap = new int[_width][];
        for (int i = 0; i < _width; i++)
        {
            basicMap[i] = new int[_height];
        }
    ...
    void selectTile()
    {
        ...
        if (m.y < GeneratedMap[0].Length && m.y >= 0 &&
           m.x < GeneratedMap.Length && m.x >= 0)
    ...
    void ChangeTile()
    {
        ...
        if (m.y < GeneratedMap[0].Length && m.y >= 0 &&
           m.x < GeneratedMap.Length && m.x >= 0)
        {
            var sprite = tiles[selectedSprite];
            GeneratedMap[(int)m.x][(int)m.y].GetComponent<SpriteRenderer>().sprite = sprite;
            GeneratedMap[(int) m.x][(int) m.y].name = sprite.name;
    ...
    public void SaveMap(string name)
    {
        ...
        FileStream file = File.Open(SaveFolderPath + name + ".dat", FileMode.Create);
    ...
    {
        var path = SaveFolderPath + name + ".dat";
        if (File.Exists(path))
        {
            Load(path);
            return;
